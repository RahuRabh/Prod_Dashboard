    ...
    <div className="bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden">
      <div className="p-5 border-b border-slate-200">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Utilization and throughput for each workstation
        </p>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-slate-500 uppercase text-xs tracking-wide">
            <tr>
              <th className="text-left px-5 py-4">Workstation</th>
              <th className="text-left px-5 py-4">Occupancy</th>
              <th className="text-left px-5 py-4">Productive Time</th>
              <th className="text-left px-5 py-4">Units</th>
              <th className="text-left px-5 py-4">Throughput</th>
          ...
          <tbody>
            {workstations.map((station) => {
              const styles = getUtilizationStyles(station.utilization);
              return (
                <tr
                  key={station.workstationId}
                  onClick={() => onSelectStation(station.workstationId)}
                ...
                >
                  ...
                  <td className="px-5 py-4 text-slate-700">
                    {(station.occupancyTime / 60).toFixed(0)} min
                  </td>

                  <td className="px-5 py-4 text-slate-700">
                    {(station.productiveTime / 60).toFixed(0)} min
                  </td>
                  ...
                  <td className="px-5 py-4 text-slate-700">
                    {station.throughputRate.toFixed(1)}/hr
                  </td>
